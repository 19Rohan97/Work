...
id`,
`name
`, `mobile`, `email`, `password`, `image`) VALUES
...
elecid`,
`Title
`, `category`, `elec_rating`, `image`, `discription`, `price`) VALUES
...
id`,
`cardnumber
`, `card_expairy`, `cvc_code`, `amount`, `userid`) VALUES
...
ALTER TABLE `cart`
  MODIFY `cartid` int
(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
ALTER TABLE `items`
  MODIFY `elecid` int
(9) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=32;
