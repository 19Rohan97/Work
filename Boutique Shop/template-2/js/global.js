...
$(function () {
  ...
  $(window).scroll(function () {
    ...
    if (winScr >= 145) {
      $("header:not(.type-3)").addClass("move");
    } else {
      $("header:not(.type-3)").removeClass("move");
    }

    if (winScr >= 115) {
      $("header.type-3").addClass("move");
    } else {
      $("header.type-3").removeClass("move");
    }
  });
      ...
      swipers["swiper-" + index] = new Swiper(".swiper-" + index, {
        ...
        onSlideChangeStart: function (swiper) {
          ...
          if ($t.parent().hasClass("swiper-chained-top")) {
            swipers[
              "swiper-" +
                $t.parent().next().find(".swiper-container").attr("id")
            ].swipeTo(activeIndex);
            $t.parent()
              .next()
              .find(".swiper-slide.chained-active")
              .removeClass("chained-active");
            $t.parent()
              .next()
              .find(".swiper-slide")
              .eq(activeIndex)
              .addClass("chained-active");
            swipers[
              "swiper-" +
                $t.parent().next().find(".swiper-container").attr("id")
            ].swipeTo(activeIndex);
        ...
        onSlideClick: function (swiper) {
          if ($t.parent().hasClass("swiper-chained-bottom"))
            swipers[
              "swiper-" +
                $t.parent().prev().find(".swiper-container").attr("id")
  ...
  $(".small-ph a").on("click", function () {
    ...
    $(".preview-image")
      ...
      .attr("data-preview", $(this).data("preview"));
    return false;
  });

  $(".preview-zoom-image").on("click", function () {
    $(".image-preview-popup .cell-view img").attr(
      "src",
      $(".small-ph a.active").data("preview")
    );
  ...
  $(".tab-switch").on("click", function () {
    ...
    var thisParent = $(this).closest(".tabs"),
      thisIndex = $(this).parent().find(".tab-switch").index(this);
    thisParent.find(".tab-switch.active").removeClass("active");
    $(this).addClass("active");
    thisParent.find(".tabs-drop").text($(this).text()).click();
    thisParent.find(".tab-wrapper:visible").fadeOut(function () {
      thisParent
        .find(".tab-wrapper")
        .eq(thisIndex)
        .fadeIn(function () {
  ...
  $(".tt-nav-tab-item").on("click", function () {
    ...
    $t.parents(".tt-tab-nav-wrapper")
      .find(".tt-tab-select select option:eq(" + index + ")")
      .prop("selected", true);
    $t.closest(".tt-tab-wrapper")
      .find(".tt-tab-info:visible")
      .fadeOut(500, function () {
        var $tabActive = $t
          .closest(".tt-tab-wrapper")
          .find(".tt-tab-info")
          .eq(index);
  ...
  $(".tt-tab-select select").on("change", function () {
    ...
    $t.closest(".tt-tab-nav-wrapper")
      .find(".tt-nav-tab-item")
      .removeClass("active");
    $t.closest(".tt-tab-nav-wrapper")
